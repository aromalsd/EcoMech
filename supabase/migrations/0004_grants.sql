...
-- Everything else stays unreachable from the client: RLS with no policies and
-- no privileges, so a policy added later still cannot expose them.
...
-- Internal SECURITY DEFINER helpers must not be callable directly, or anyone
-- could rewrite derived state or move another device's reputation.
